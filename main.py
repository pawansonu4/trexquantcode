...
class HangmanGame(object):
    ...
    def getword(self):
        ...
        file_path = "words_alpha.txt"  # Replace with the actual file path

        # Read words from the file
        english_words = read_words_from_file(file_path)
    ...
    def start_game(self, num_lives,word, verbose=True):

        print("please enter a word for the computer to guess:")
...
if __name__ == "__main__":
    ...
    randomwords = game.getword()
    success = 0

    for word in randomwords:
        args.word = word
        check = game.start_game(args.lives,args.word)
        if check == True:
            success +=1
    print("success rate")
    print(success/len(randomwords)*100)